...
::: warning Rails 側で `api` ホストを許可する(1 行)
`VITE_API_PROXY_TARGET: http://api:3000` を使うと、Vite は Rails へ **`Host: api:3000`** でリクエストを転送します。Rails は開発環境でも `ActionDispatch::HostAuthorization` で許可ホストを絞っており、`api` は未許可なので、そのままだとサインアップ時に次のエラーで弾かれます。

```
[ActionDispatch::HostAuthorization::DefaultResponseApp] Blocked hosts: api:3000
```

第4章で作った `api/config/environments/development.rb` に 1 行足して、Docker のサービス名を許可します。

```ruby
# Vite proxy(frontend → api:3000)のホスト名を許可する
config.hosts << "api"
```

反映には API コンテナの再起動が必要です。

```bash
docker compose restart api
```

(コンテナを使わずローカルの `localhost:3000` に転送する場合は、ホストが `localhost` なのでこの設定は不要です。)
:::
...
export class ApiError extends Error {
  ...
  constructor(status: number, body: { error?: string; errors?: string[] } | null) {
...
::: tip なぜ `public status` と書かないのか(erasableSyntaxOnly)
上の `ApiError` は `constructor(public status, ...)` という**パラメータープロパティ**(引数に `public` を付けてフィールド宣言と代入を同時に済ませる短縮記法)を使いたくなる場面です。しかし今の Vite の react-ts テンプレートが生成する `tsconfig.app.json` には `"erasableSyntaxOnly": true` が入っており、この記法は**構文エラーになります**。

このオプションは「型を消すだけで JS になる構文」しか許しません。パラメータープロパティ・`enum`・`namespace` の実装などは代入や実オブジェクトを**生成する**ため対象外です(esbuild/Node の高速な型ストリップ前提の設定)。なので上のようにフィールドを明示宣言し、コンストラクタ内で `this.status = status` と代入する形で書きます。
:::
...
| `Blocked hosts: api:3000`(API ログ) | Rails のホスト制限。development.rb に `config.hosts << "api"` を追加し `docker compose restart api`(Step 3 の warning 参照) |
| `api.ts` の `public status` で構文エラー | `erasableSyntaxOnly` はパラメータープロパティを禁止。フィールドを明示宣言して代入する(Step 4 の tip 参照) |
