...
docker compose restart api          # ← 追加された bcrypt を稼働中サーバーに反映
docker compose run --rm api bin/rails db:migrate
```

::: warning bundle install だけでは反映されない
`bundle install` は gem をインストールするだけで、**すでに起動しているサーバー(`docker compose up` のプロセス)には反映されません**。反映するにはコンテナの再起動が必要です。これを忘れると、後で `LoadError: cannot load such file -- bcrypt` が出ます(後述の「よくあるトラブル」参照)。
:::
...
生成コードはビュー(HTML 画面)がある通常の Rails アプリ前提です。API モード(`ActionController::API`)向けに、次の 5 つを順に直します。

#### ① クッキーミドルウェアを追加する(最重要)

まずここが本章で一番ハマりやすい所です。API モードは「クッキーを扱うミドルウェア(`ActionDispatch::Cookies`)」がスタックから**外されています**。これが無いと、`cookies.signed[...] = ...` を実行しても `Set-Cookie` ヘッダがレスポンスに書き出されず、ログインしてもクッキーが発行されません(=ずっと 401)。

`api/config/application.rb` の `config.api_only = true` の直後に追記します:
...
::: warning include だけでは足りない・両方いる
次の ② の `include ActionController::Cookies` は「コントローラ内で `cookies` を**呼べる**ようにする」だけで、`Set-Cookie` を実際に**書き出す**のはこのミドルウェアの仕事です。**「コントローラ側の include」と「ミドルウェアの追加」は両方必要**、と覚えてください。`config/application.rb` はミドルウェア変更なので、保存後は `docker compose restart api` で再起動が必要です。
:::

反映されたかは次で確認できます:

```bash
docker compose exec api bin/rails middleware | grep -i cookie
# → use ActionDispatch::Cookies  と表示されれば OK
```

#### ② ApplicationController でクッキーを include する
...
#### ③ `helper_method` の行を削除する

生成された `app/controllers/concerns/authentication.rb` の `included do` 内には、次の行があります:

```ruby
  included do
    before_action :require_authentication
    helper_method :authenticated?   # ← この行を削除する
  end
```

`helper_method` は「コントローラのメソッドをビュー(ERB)から呼べるようにする」ためのもので、ビューを持たない `ActionController::API` には**存在しません**。残したままだと、include した瞬間に `undefined method 'helper_method' for class ApplicationController` で**全リクエストが 500** になります。ビューを使わない API では不要なので、この行を削除します。

#### ④ 未認証時はリダイレクトではなく 401 を返す

`app/controllers/concerns/authentication.rb` の `request_authentication`(生成コードはログイン画面へリダイレクトする)を、次の 1 メソッドに置き換えます:
...
ログイン中ユーザーを取り出す便利メソッドを、concern の private 内に足しておきます:
...
| 症状 | 原因と対処 |
| --- | --- |
| `undefined method 'helper_method' for class ApplicationController` で全リクエストが 500 | 生成コードの `helper_method :authenticated?` が残っている。API モードには無いメソッドなので**この行を削除**(Step 3 ③) |
| `LoadError: cannot load such file -- bcrypt`(または `undefined method 'authenticate'`) | bcrypt が稼働中サーバーに未ロード。`docker compose run --rm api bundle install` 後、**必ず `docker compose restart api` で再起動** |
| ログインは 201 なのに `Set-Cookie` が出ず、以降ずっと 401 | API モードで `config.middleware.use ActionDispatch::Cookies` を付け忘れている(Step 3 ①)。追加後にコンテナ再起動 |
| 一度ログインできたのに、その後急に 401 になった | 失敗レスポンス(例: 422)を `-c cookie.txt` で叩くと、`Set-Cookie` の無いレスポンスで **cookie.txt が空に上書き**される。ログインし直して取り直す(保存したいリクエストだけ `-c` を使う) |
| `undefined method 'cookies'` | `ApplicationController` に `include ActionController::Cookies` を忘れている(Step 3 ②) |
